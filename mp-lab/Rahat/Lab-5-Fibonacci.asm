...
.DATA    
    ...
    MSG2    DB  10,13,'The Fibonacci series: 000 001 $'
...
NEXT:
    ...
    MOV AH, 9
    LEA DX, MSG2            ; printing the message with 0 & 1
    INT 21H
    
    SUB CL, 2               ; decrementing 2 as the first two number has already been displayed
    
CALCULATE: 

    MOV AL, A               
    ADD AL, B               ; add the previous 2 number A & B
    MOV C, AL                   ; store the summation in C
    ...
    MOV AL, B               
    MOV A, AL               ; store B in A
    MOV AL, C
    MOV B, AL               ; store C in B
